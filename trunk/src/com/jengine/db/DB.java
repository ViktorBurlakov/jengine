...
public class DB {
    ...
    public Model get(Class cls, Object id) throws DBException, SystemException, PortalException {
    ...
    public ModelQuery filter(Class cls, String query, Object ... params) throws SystemException {
        return new ModelQuery(getManager(cls)).filter(query, (Object[]) params);
    }

    public ModelQuery filter(Class cls, Map<String, Object> filter) throws SystemException {
        return new ModelQuery(getManager(cls)).filter(filter);
    }

    public ModelQuery filter(Class cls, Expression... filters) throws SystemException {
    ...
    public Long count(Class cls) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return (Long) new ModelQuery(manager).field(manager.getCountField(manager.getPrimaryKey().getName())).one();
    }

    public Long count(Class cls, String field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return (Long) new ModelQuery(manager).field(manager.getCountField(field)).one();
    }

    public Object max(Class cls, String field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getMaxField(field)).one();
    }

    public Object min(Class cls, String field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getMinField(field)).one();
    }

    public Object avg(Class cls, String field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getAvgField(field)).one();
    }

    public Object sum(Class cls, String field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getSumField(field)).one();
    }

    public Long count(Class cls, Field field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return (Long) new ModelQuery(manager).field(manager.getCountField(field)).one();
    }

    public Object max(Class cls, Field field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getMaxField(field)).one();
    }

    public Object min(Class cls, Field field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getMinField(field)).one();
    }

    public Object avg(Class cls, Field field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getAvgField(field)).one();
    }

    public Object sum(Class cls, Field field) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        return new ModelQuery(manager).field(manager.getSumField(field)).one();
    }

    public Object calc(Class cls, String name, Class type, String expr, Field ... fields) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
    ...
    public void remove(Class cls) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(cls);
        new ModelQuery(manager).remove();
        clearCache(cls);
    }

    public Model cache(Model obj) throws SystemException, PortalException {
    ...
    public Model getCache(Class cls, Object id) throws SystemException, PortalException {
        return getManager(cls).getCacheEnabled() ? provider.getCache(cls, id) : null;
    }

    public void clearCache(Model obj) throws SystemException, PortalException {
        if (getManager(obj.getClass()).getCacheEnabled()) {
            provider.clearCache(obj);
        }
    }

    public void clearCache(Class cls) throws SystemException, PortalException {
        if (getManager(cls).getCacheEnabled()) {
            provider.clearCache(cls);
        }
    }

    public Model insert(Model obj) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(obj.getClass());
        obj.setValue(manager.getPrimaryKey(), provider.newPrimaryKey(obj.getClass()));
        insert(new ModelQuery(manager).values(obj.getValues(manager.getFieldNames())));
        return cache(obj);
    }

    public Model update(Model obj) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(obj.getClass());
        new ModelQuery(manager).values(obj.getValues(manager.getFieldNames())).update();
        return cache(obj);
    }

    public void remove(Model obj) throws SystemException, PortalException, DBException {
        ModelManager manager = getManager(obj.getClass());
        new ModelQuery(manager).values(obj.getValues(manager.getFieldNames())).remove();
        clearCache(obj);
    }

    public Model save(Model obj) throws SystemException, PortalException, DBException {
        if (obj.isNew()) {
            return insert(obj);
        }
        return update(obj);
    ...
    public List run(ModelQuery modelQuery) throws SystemException, PortalException, DBException {
        List values = provider.select(modelQuery);
        return processResult(modelQuery, values);
    }

    protected List processResult(ModelQuery modelQuery, List values) throws SystemException, PortalException, DBException {
    ...
    protected List processResultItem(ModelQuery modelQuery, Object[] items) throws DBException, SystemException, PortalException {
        List result = new ArrayList();
        int index=0;
